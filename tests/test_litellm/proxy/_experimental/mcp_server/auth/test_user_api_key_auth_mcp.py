...
class TestUserSubjectTeamUnion:
    ...
    async def test_every_granting_team_rpm_limit_binds_on_the_union(self):
        """A key is pinned to one team so the limiter reads its team_metadata. This subject has NO
        team_id, so before this every team's mcp_rpm_limit silently did not apply and a cross-team
        user outran all of them. Each granting team is charged rather than one being picked, so a
        server granted by several teams has no arbitrary attribution."""
        ...
        assert auth.mcp_source_team_rpm_limits == {"t1": {"srv1": 5}, "t2": {"srv1": 9}}
        ...
        assert charged == {"t1:srv1": 5, "t2:srv1": 9}, "every granting team's bucket must be charged"
    ...
    def _patch(self, *, teams_by_id, user_teams, org_perms=None, registry=None):
        ...
        async def _get_org_object(org_id, **kw):
            if org_id not in org_perms:
                raise Exception(f"Organization doesn't exist. Org={org_id}.")  # matches production
            op = org_perms[org_id]
