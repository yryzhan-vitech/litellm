...
class S3Logger(CustomBatchLogger, BaseAWSLLM):
    ...
    def _encode_s3_object_key(s3_object_key: str) -> str:
        # [ARC-BUG-10] Encode the object key once (preserving "/" separators) so
        # keys with reserved chars (space, #, +, &, unicode) don't 403. Also
        # percent-encode "."/".." segments: httpx collapses them on the wire but
        # S3SigV4Auth would sign them as-is -> signed path != wire path.
        encoded_segments = []
    ...
    def _sign_s3_request(
        ...
        # [ARC-BUG-10] Sign with S3SigV4Auth (not generic SigV4Auth): the generic
        # signer re-quotes the path (%20 -> %2520) so the signed canonical URI no
        # longer matches the URL httpx sends -> 403 SignatureDoesNotMatch.
        try:
    ...
    async def async_upload_data_to_s3(self, batch_logging_element: s3BatchLoggingElement):
        ...
        try:
            ...
            # [ARC-BUG-10] Send the exact same (already-encoded) URL that was signed.
            request_url = url
