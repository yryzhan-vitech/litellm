...
class AdvisorInterceptionLogger(CustomLogger):
    ...
    def __init__(
        self,
        enabled_providers: list[Union[LlmProviders, str]] | None = None,
        default_advisor_model: str | None = None,
    ):
        super().__init__()
    ...
    @classmethod
    def from_config_yaml(cls, config: AdvisorInterceptionConfig) -> "AdvisorInterceptionLogger":
        """
    ...
    async def async_pre_call_deployment_hook(self, kwargs: dict[str, Any], call_type: Any | None) -> dict | None:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        ...
        if not custom_llm_provider:
            ...
            except Exception:  # noqa: BLE001  # ported verbatim; interception must never break the caller's completion
                custom_llm_provider = ""
        return self._convert_tools_for_provider(kwargs=kwargs, custom_llm_provider=custom_llm_provider)
    ...
    async def async_post_call_success_deployment_hook(
        ...
        response: Any,  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        call_type: CallTypes | None,
    ) -> Any | None:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        """
        ...
        converted_stream = isinstance(call_id, str) and call_id in self._converted_stream_call_ids
        if converted_stream:
            assert isinstance(call_id, str)  # guaranteed by converted_stream check above
        ...
        custom_llm_provider = request_data.get("custom_llm_provider", "") or request_data.get("litellm_params", {}).get(
            "custom_llm_provider", ""
        )
        if not custom_llm_provider:
            try:
    ...
    async def async_should_run_chat_completion_agentic_loop(
        ...
        if custom_llm_provider in ADVISOR_NATIVE_PROVIDERS:
            ...
            advisor_cfg = (
                self._advisor_config_by_call_id.get(call_id_check, {}) if isinstance(call_id_check, str) else {}
            )
        ...
        call_id = kwargs.get("litellm_call_id")
        has_advisor_config = isinstance(call_id, str) and (call_id in self._advisor_config_by_call_id)
        tools_list: list[dict] = tools or []
        has_advisor_tool = has_advisor_config or (
            bool(tools_list) and any(is_advisor_tool_chat_completion(t) for t in tools_list)
        )
        ...
        if len(advisor_calls) != len(raw_tool_calls):
            verbose_logger.debug("AdvisorInterception: Mixed tool calls detected, skipping advisor interception")
            if isinstance(call_id, str):
                self._advisor_config_by_call_id.pop(call_id, None)
            return False, {}
    ...
    async def async_run_chat_completion_agentic_loop(
        ...
        logging_obj: Any,  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        stream: bool,
        kwargs: dict,
    ) -> Any:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        """
        ...
        try:
            while True:
                advisor_calls, raw_tool_calls = self._extract_advisor_tool_calls(current_response)
                if not advisor_calls:
                    self._set_response_cost_if_possible(response=current_response, response_cost=total_response_cost)
                    self._inject_advisor_results_into_response(current_response, advisor_interactions)
                ...
                for advisor_call in advisor_calls:
                    ...
                    question = advisor_call.get("question", "Please provide guidance on the current task.")
    ...
    @staticmethod
    def _wrap_as_streaming_if_needed(response: Any) -> Any:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        """
    ...
    @staticmethod
    def _get_llm_router() -> Any | None:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
    ...
    def _is_native_anthropic_advisor_model(advisor_model: str) -> bool:
        """
        Return True when the advisor model resolves to an Anthropic Claude Opus
        model supported as a native advisor (currently Opus 4.6 or 4.7).

        Handles bare model names, litellm provider-prefixed names
        (e.g. ``anthropic/claude-opus-4-7``) and proxy model aliases.
        """
            ...
            if llm_router is not None:
                for deployment in llm_router.model_list or []:
                    if deployment.get("model_name") == advisor_model:
                        advisor_model = deployment.get("litellm_params", {}).get("model") or advisor_model
                        break
        except Exception:  # noqa: BLE001  # ported verbatim; interception must never break the caller's completion
            pass

        normalized = advisor_model.lower().replace("_", "-")
        is_anthropic = normalized.startswith("anthropic/") or ("/" not in normalized and "claude" in normalized)
        is_native_opus = any(
            token in normalized
            for token in (
                "claude-opus-4-6",
        ...
        return is_anthropic and is_native_opus

    @staticmethod
    async def _call_advisor_model(
        llm_router: Any | None,  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        advisor_model: str,
        messages: list[dict],
        max_tokens: int,
    ...
    def _convert_tools_for_provider(self, kwargs: dict[str, Any], custom_llm_provider: str) -> dict[str, Any] | None:
        if self.enabled_providers is not None and custom_llm_provider not in self.enabled_providers:
            return None
        ...
        use_native = custom_llm_provider in ADVISOR_NATIVE_PROVIDERS and self._is_native_anthropic_advisor_model(
            advisor_model
        )
        if use_native:
            for tool in tools:
    ...
    def _extract_advisor_config(self, tools: list[dict]) -> dict[str, Any]:
        ...
        for tool in tools:
            ...
            if tool.get("type") == "advisor_20260301":
                ...
                # api_key / api_base in the request body are ignored: they would
                # let an authenticated caller redirect advisor sub-calls (plus
                # any resolved provider credentials) to an attacker-controlled
                # host. Credentials must come from model_list via the router.
    ...
    def _extract_advisor_tool_calls(self, response: Any) -> tuple[list[dict], list[dict]]:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
    ...
    @staticmethod
    def _inject_advisor_results_into_response(response: Any, advisor_interactions: list[dict[str, str]]) -> None:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        """
    ...
    @staticmethod
    def _extract_first_choice_message_obj(response: Any) -> Any:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
    ...
    @staticmethod
    def _extract_first_choice_message(response: Any) -> dict | None:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        if isinstance(response, dict):
            choices = response.get("choices", [])
        else:
            choices = getattr(response, "choices", None) or []
    ...
    @staticmethod
    def _extract_message_content(response: Any) -> str | None:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
    ...
    @staticmethod
    def _extract_text_content(response: Any) -> str:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        message = AdvisorInterceptionLogger._extract_first_choice_message(response)
        if not message:
            return ""
    ...
    @staticmethod
    def _safe_get_response_cost(response: Any) -> float:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        if response is None:
            return 0.0
    ...
    @staticmethod
    def _set_response_cost_if_possible(response: Any, response_cost: float) -> None:  # noqa: ANN401  # ported verbatim from fork main / prod image a2e1b93d30; upstream signature
        if response is None:
            return
        if response_cost <= 0:
            return
    ...
    def _prepare_followup_kwargs(kwargs: dict) -> dict:
        ...
        return {
            k: v for k, v in kwargs.items() if not k.startswith("_advisor_interception") and k not in internal_params
        }
