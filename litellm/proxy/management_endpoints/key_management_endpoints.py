...
# [ARC-BUG-06] sync the cross-pod key spend counter on explicit spend or window-reset (upstream PR #34493)
...
# [ARC-BUG-06] audit payload carries only server-inferred budget fields, never the whole
# non_default_values (prepare_metadata_fields injects unchanged metadata) (upstream PR #34493)
...
# [ARC-BUG-06] reset carried spend when a key's budget window is (re)armed on update (upstream PR #34493)
...
async def _process_single_key_update(
    ...
    asyncio.create_task(
        KeyManagementEventHooks.async_key_updated_hook(
            ...
            user_api_key_dict=user_api_key_dict,
            litellm_changed_by=litellm_changed_by,
            # [ARC-BUG-06] record inferred spend reset in the audit log (upstream PR #34493)
...
async def _validate_update_key_data(
    ...
    _is_budget_change = (
        ...
        # [ARC-BUG-06] budget_duration re-arm resets spend, so gate it as a budget change (upstream PR #34493)
    ...
    *,
    ...
    asyncio.create_task(
        KeyManagementEventHooks.async_key_rotated_hook(
            ...
            user_api_key_dict=user_api_key_dict,
            litellm_changed_by=litellm_changed_by,
            # [ARC-BUG-06] record the inferred spend reset / armed window (upstream PR #34493)
            applied_values=_budget_window_audit_values(non_default_values),
        )
    )
    return response
