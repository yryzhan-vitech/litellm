...
class MCPRequestHandler:
    ...
    async def _admitted_subject_team_rpm_limits(auth: UserAPIKeyAuth) -> dict[str, dict[str, int]] | None:
        ...
        try:
            ...
            for source, granted_ids in await MCPRequestHandler.admitted_source_grants(auth):
                ...
                applicable = {
                    ...
                    if granted_ids & set(global_mcp_server_manager.expand_permission_list([server_name]))
                }
                if applicable:
    ...
    @staticmethod
    async def attributing_source_for_server(auth: UserAPIKeyAuth, server_id: str) -> UserAPIKeyAuth | None:
    ...
    async def _resolve_admitted_subject_tools(server_id: str, auth: UserAPIKeyAuth) -> list[str] | None:
        ...
        # A server reachable through an OPEN channel (operator-opened allow_all_keys, the user's own
        # BYOM submission) is default-open for tools, exactly as a virtual key experiences it — no
        # grant source names it, so the source union below would return [] and leave the server
        # listable but uninvokable. Same owner as the server union, so the two axes cannot diverge.
        if server_id in await global_mcp_server_manager.operator_open_server_ids(auth):
            return None
    ...
    async def _get_org_object_permission(
        ...
        try:
            ...
            verbose_logger.debug(f"MCP org ceiling: org row {user_api_key_auth.org_id!r} not readable: {e}")
            return None
